...
if __name__ == '__main__':
    ...
    # If SQS empty then list buckets and create jobs
    if check_sqs_empty(sqs, sqs_queue):
        logger.info('Job sqs queue is empty, now process comparing s3 bucket...')

        # Load Bucket para from local jobsender_bucket_list.txt
        logger.info(f'Load Bucket para from local jobsender_bucket_list.txt')
        try:
            jobsender_bucket_list_path = os.path.split(os.path.abspath(__file__))[0] + '/jobsender_bucket_list.txt'
            with open(jobsender_bucket_list_path, 'r') as f:
                load_bucket_para = json.load(f)
        except Exception as e:
            print(f'Fail to get buckets info from local jobsender_bucket_list.txt {str(e)}')
            sys.exit(0)
